...
EPIC=$(n "WireGuard overlay network" --status in_progress --tags infra,epic:overlay,area:net --created-at "2026-08-20 09:00" --body "## Requirements
...
IMPL=$(n "Implement WireGuard overlay" --status in_progress --tags infra,area:net --parent "$EPIC" --assignee claude --created-at "2026-08-22 10:00" --body "## Requirements
...
DISC=$(n "Peer discovery via DNS" --status review --tags infra,area:net --parent "$EPIC" --assignee sonnet --created-at "2026-08-25 14:00" --body "## Requirements
...
n "Rotate keys on a schedule" --status idea --tags infra,security,area:net --parent "$EPIC" --created-at "2026-08-28 11:00" >/dev/null
n "Write the network docs" --status ready --tags docs,area:docs --blocked-by "$IMPL" --created-at "2026-08-26 09:30" >/dev/null
CFG=$(n "Design the config format" --status plan --tags design,area:platform --created-at "2026-08-27 16:00" --body "## Requirements
...
HEALTH=$(n "Add a health endpoint" --status ready --tags ops,area:platform --created-at "2026-08-29 08:15")
n "Prometheus exporter for tunnel metrics" --status idea --tags observability,area:platform --created-at "2026-09-01 12:00" >/dev/null
n "Set up CI" --status done --tags ops,area:platform --created-at "2026-08-18 10:00" >/dev/null
DB=$(n "Choose the state store" --status done --tags design,area:platform --created-at "2026-08-19 10:00")
MULTI=$(n "Multi-region routing" --status plan --tags infra,epic:overlay,area:net --created-at "2026-09-02 10:00" --body "## Requirements
