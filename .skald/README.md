<!-- skald-render 3f42b70cc02ed586 -->
# skald backlog

**13 open** · Backlog 1 · Ready 8 · In progress 0 · Review 4 · Done 0

Rendered by [Skald](https://github.com/Vitund-AI/skald) from the story files in this directory. Regenerate with `skald render`.

## Backlog (1)

| ID | Title | Tags | Assignee | Blocked by | Progress |
| --- | --- | --- | --- | --- | --- |
| [da2a34](stories/da2a34-parents-on-the-board-and-in-history-children-in-th.md) | Parents on the board and in history: children in the modal, epics merge, commits union | `git` `ui` |  |  | 0/2 |

## Ready (8)

| ID | Title | Tags | Assignee | Blocked by | Progress |
| --- | --- | --- | --- | --- | --- |
| [b91662](stories/b91662-publish-0-2-0-to-pypi.md) | Publish 0.2.0 to PyPI | `packaging` |  |  |  |
| [cb3bd6](stories/cb3bd6-questions-as-a-note-kind-open-until-a-later-decisi.md) | Questions as a note kind: open until a later decision, surfaced in context, resume, ls, and the board with a waiting-on-a-human filter | `agents` `core` `ui` |  |  | 0/4 |
| [32076e](stories/32076e-lifecycle-columns-idea-and-plan-before-ready-an-in.md) | Lifecycle columns: idea and plan before ready, an init preset, and a plan-to-ready warning on open questions | `core` `docs` |  |  | 0/3 |
| [4798f4](stories/4798f4-document-the-design-record-layout-long-stories-sec.md) | Document the design-record layout: long stories, sections the tools understand, a template | `docs` |  |  | 0/2 |
| [67f581](stories/67f581-lanes-facet-limits-so-stories-that-must-not-run-co.md) | Lanes: facet limits so stories that must not run concurrently are not picked together | `agents` `core` |  |  | 0/4 |
| [cff9e4](stories/cff9e4-skald-audit-check-a-story-s-paths-line-references.md) | skald audit: check a story's paths, line references, and commit hashes against the tree, and note it | `agents` `cli` |  |  | 0/3 |
| [fe6d27](stories/fe6d27-parents-a-parent-field-so-an-epic-can-have-a-body.md) | Parents: a parent field so an epic can have a body and children | `cli` `core` |  |  | 0/3 |
| [fcecad](stories/fcecad-backdating-flags-note-at-and-new-created-at-for-mi.md) | Backdating flags: note --at and new --created-at for migration scripts | `cli` |  |  | 0/2 |

## In progress (0)

_none_

## Review (4)

| ID | Title | Tags | Assignee | Blocked by | Progress |
| --- | --- | --- | --- | --- | --- |
| [a25045](stories/a25045-serve-port-0-falls-back-to-the-configured-port.md) | serve --port 0 falls back to the configured port | `core` | claude |  | 3/3 |
| [f3bac3](stories/f3bac3-claude-code-sessionstart-hook-runs-skald-context-n.md) | Claude Code SessionStart hook runs skald context, not skald ls | `agents` | claude |  | 3/3 |
| [2f74dd](stories/2f74dd-remove-the-0-1-migration-path-and-its-mentions.md) | Remove the 0.1 migration path and its mentions | `cli` `docs` | claude |  | 3/3 |
| [45f306](stories/45f306-skald-resume-at-the-right-altitude-requirements-se.md) | skald resume at the right altitude: requirements section only, table of contents, --section and --full | `agents` `cli` | claude |  | 4/4 |

<details><summary><strong>Done (0)</strong></summary>

_none_

</details>
