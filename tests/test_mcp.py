...
class TestMcp(SkaldTestCase):
    ...
    def test_tools_end_to_end(self):
        ...
        self.call(server, "skald_note", id=a, text="And auth?", kind="question")
        ans, _ = self.call(server, "skald_answer", id=a, text="5000", question="Q1", **{"as": "jon"})
        self.assertEqual((ans["closed_questions"], ans["questions"]["open"]), (["Q1"], 1))
        ans, _ = self.call(server, "skald_answer", id=a, text="cookies", withdraw=True, **{"as": "jon"})
        self.assertEqual((ans["closed_questions"], ans["questions"]["open"]), (["Q2"], 0))
        self.assertEqual(ans["questions"]["closed"][1]["closed_by"]["how"], "withdrawn")
        old, _ = self.call(server, "skald_new", title="old", created_at="2024-01-01 08:00")
        self.assertEqual(old["story"]["created_at"], "2024-01-01T08:00:00Z")
        dated, _ = self.call(server, "skald_note", id=old["story"]["id"], text="then", at="2024-01-02 08:00")
        self.assertIn("## [agent] 2024-01-02 08:00 UTC", dated["body"])
        kid, _ = self.call(server, "skald_new", title="child", parent=a)
        self.assertEqual(kid["story"]["parent"], a)
        cleared, _ = self.call(server, "skald_set", id=kid["story"]["id"], parent="-")
        self.assertNotIn("parent", cleared["story"])
