...
class TestAPI(ServerTestCase):
    ...
    def test_crud_flow(self):
        ...
        status, data = self.call("POST", f"{P}/stories/{a}/notes", {"text": "Which port?", "kind": "question"})
        ...
        # A decision aimed at one question (as the board's per-question Answer does) closes only that one.
        status, data = self.call("POST", f"{P}/stories/{a}/notes", {"text": "And the host?", "kind": "question", "author": "claude"})
        self.assertEqual(data["questions"]["open"], 2)
        status, data = self.call("POST", f"{P}/stories/{a}/notes", {"text": "5000", "kind": "decision", "question": "Q2"})
        self.assertEqual(status, 201)
        self.assertEqual([(q["number"], q["text"]) for q in data["questions"]["items"]], [(1, "Which port?")])
        self.assertIn("· decision\nAnswers Q2 [claude] ", data["body"])
        self.assertIn(" · And the host?\n5000\n", data["body"])
        self.assertEqual(self.call("POST", f"{P}/stories/{a}/notes", {"text": "x", "kind": "decision", "question": 3})[0], 400)
        self.assertEqual(self.call("POST", f"{P}/stories/{a}/notes", {"text": "x", "kind": "handoff", "question": 1})[0], 400)
        self.assertEqual(self.call("POST", f"{P}/stories/{a}/notes", {"text": "x", "question": "nope"})[0], 400)
        # A plain decision closes nothing; "all" (the board's Answer all) closes every open question.
        status, data = self.call("POST", f"{P}/stories/{a}/notes", {"text": "thinking", "kind": "decision"})
        self.assertEqual(data["questions"]["open"], 1)
        status, data = self.call("POST", f"{P}/stories/{a}/notes", {"text": "5000", "kind": "decision", "all": True})
        self.assertEqual(data["questions"]["open"], 0)
        self.assertIn("· decision\nAnswers Q1 [", data["body"])
        ...
        # Parents over the API: create with parent (inheriting facets), read the family, patch it, and history unions children.
        self.call("PATCH", f"{P}/stories/{a}", {"tags": ["epic:x", "plain"]})
        status, data = self.call("POST", f"{P}/stories", {"title": "piece", "parent": a})
        self.assertEqual((status, data["story"]["parent"], data["story"]["tags"]), (201, a, ["epic:x"]))
        kid = data["story"]["id"]
        status, one = self.call("GET", f"{P}/stories/{a}")
        self.assertEqual([c["id"] for c in one["children"]], [kid])
        self.assertEqual(one["children"][0]["done"], False)
        status, k = self.call("GET", f"{P}/stories/{kid}")
        self.assertEqual(k["parent_story"]["id"], a)
        status, board = self.call("GET", f"{P}/board")
        self.assertEqual(next(s for s in board["stories"] if s["id"] == a)["children"], {"total": 1, "done": 0})
        git(self.repo, "add", "-A")
        git(self.repo, "commit", "-qm", f"kid work\n\nSkald-Story: {kid}")
        status, hist = self.call("GET", f"{P}/stories/{a}/history")
        self.assertEqual((hist["children"], [c["story"] for c in hist["commits"]]), ([kid], [kid]))
        status, hist = self.call("GET", f"{P}/stories/{a}/history?children=0")
        self.assertEqual(hist["commits"], [])
        status, data = self.call("PATCH", f"{P}/stories/{kid}", {"parent": "-"})
        self.assertNotIn("parent", data["story"])
