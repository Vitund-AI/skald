...
class TestAPI(ServerTestCase):
    ...
    def test_crud_flow(self):
        ...
        status, data = self.call("POST", f"{P}/stories/{a}/notes", {"text": "5000", "kind": "decision", "question": 2})
        self.assertEqual(status, 201)
        self.assertEqual([q["text"] for q in data["questions"]["items"]], ["Which port?"])
        self.assertIn("· decision\nAnswers [claude] ", data["body"])
        ...
        self.assertEqual(self.call("POST", f"{P}/stories/{a}/notes", {"text": "x", "question": "1"})[0], 400)
