...
class TestCompletion(SkaldTestCase):
    ...
    def test_refs_facets_config_projects_hooks(self):
        ...
        self.assertIn("features.claude_code_link", self.values("config", ""))
        self.assertEqual(self.values("config", "push", ""), ["true", "false"])
        self.assertEqual(self.values("config", "features.claude_code_link", ""), ["true", "false"])
        ...
        self.assertEqual(self.values("server", ""), ["start", "stop", "restart", "status", "token"])
